...
def time_of_meridian_by_azim(date_dt):  # date_dt is a datetime
    ...
    iters = 0
    while abs(error_deg) >= 1/48000 : # 1/4800 : # 0.05s = 1/20 / (4 * 60) : get to within 0.05 sec
        iters += 1
...
def time_of_meridian_by_eot_at_noon(date_dt):  # date_dt is a datetime
...
for i in range(0, (2 * window_weeks + 1) * 7, interval_days):  # step in weeks, starting on a Sunday
    ...
    (meridian_dt, eot_string, dec_degrees, dec_string) = time_of_meridian_by_transit(date_dt) # rhodesmill method
    (azim_noon_dt, eot_string, dec_degrees, dec_string) = time_of_meridian_by_eot_at_noon(date_dt) # add noon azimuth method
    ...
    merid_eot_s = meridian_dt.second + meridian_dt.microsecond/1000000
    ...
    print(f"Transit method: {merid_eot_s:10.5f}  Azimuth method: {azim_eot_s:10.5f}  Diff: {(merid_eot_s-azim_eot_s):10.5f} secs")
    print(f"Transit method: {merid_eot_s:10.5f}  Noon    method: {noon_eot_s:10.5f}  Diff: {(merid_eot_s-noon_eot_s):10.5f} secs")
